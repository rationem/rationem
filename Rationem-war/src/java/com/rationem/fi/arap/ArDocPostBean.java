...
public class ArDocPostBean extends BaseBean {
 ...
 private void init(){
  ...
  if(getCompList() == null){
   ...
   FiscalPeriodYearRec fiscPer = buffer.getCompFiscalPeriodYearForDate(docCompany, currDate);
   LOGGER.log(INFO, "fiscPer year {0} period {1}", new Object[]{fiscPer.getYear(),fiscPer.getPeriod()});
   fiDocument.setFisPeriod(fiscPer.getPeriod());
   fiDocument.setFisYear(fiscPer.getYear());
   fiDocument.setFisYearPeriod(fiscPer.getYearPeriod());
  }
  ...
  if(currView.equals("arInvPost")|| StringUtils.equals(currView, "arInvPostOld")){
   ...
   LineTypeRuleRec arLnTyp = buffer.getLineTypeRuleByCode("AR");
   
   docLineAr.setLineType(arLnTyp);
   LOGGER.log(INFO, "ar Line type {0}", docLineAr.getLineType().getLineCode());
 ...
 private List<DocLineGlRec> buildInvReconLines(DocLineArRec arLine,List<FundBalance> fndbal,
   DocFiRec apDoc, LineTypeRuleRec lnTyGl ){
  LOGGER.log(INFO, "getInvReconLines apLine {0} fundBal {1}", new Object[]{arLine,fndbal} );
  List<DocLineGlRec> retList = new ArrayList<>();
  if(fndbal == null || fndbal.isEmpty()){
   DocLineGlRec recLine = buildReconGlLine(arLine, apDoc, lnTyGl);
   recLine.setDocAmount(arLine.getDocAmount());
   retList.add(recLine);
  }else{
 ...
 private DocLineGlRec buildReconGlLine(DocLineArRec arLine, DocFiRec apDoc,LineTypeRuleRec lnTyGl){
  
  FiGlAccountCompRec recAcnt = arLine.getArAccount().getReconciliationAc();
  DocLineGlRec glReconLnRec = new DocLineGlRec();
  glReconLnRec.setGlAccount(recAcnt);
  //apRec.setDocAmount(apLine.getDocAmount());
  glReconLnRec.setAccountRef(recAcnt.getCoaAccount().getRef());
  glReconLnRec.setSortOrder(arLine.getSortOrder());
  glReconLnRec.setComp(arLine.getComp());
  glReconLnRec.setCreateBy(arLine.getCreateBy());
  glReconLnRec.setCreateDate(arLine.getCreateDate());
  glReconLnRec.setDocFi(apDoc);
  glReconLnRec.setLineText(arLine.getLineText());
  glReconLnRec.setLineType(lnTyGl);
  glReconLnRec.setAutoGenerated(true);
  // determine post type from arLIne
  PostTypeRec ptRec;
  if(arLine.getPostType().isDebit()){
   ptRec = buffer.getPostTypeForCode("glDrRecon");
  }else{
   ptRec = buffer.getPostTypeForCode("glCrRecon");   
  }
 ...
 public List<FundBalance> getFundBalForDoc(List<DocLineBaseRec> lines){
  ...
  for(DocLineBaseRec curr: lines){
   ...
   if(curr.getClass().getSimpleName().equals("DocLineGLRec")){
    ...
    double lineDocAmount = currLineGl.getDocAmount();
    if(!currLineGl.getPostType().isDebit()){
      lineDocAmount *= -1; 
    }
    LOGGER.log(INFO, "docAmnt {0}", lineDocAmount);
...
public void onArInvStepBack(){
 ...
 LOGGER.log(INFO, "Ar line text {0}", docLineAr.getLineText());
 int curr = getStep();
...
public void onArInvLineDelete(){
 LOGGER.log(INFO, "onArInvLineAdd called");
 ListIterator<DocLineBaseRec> li = fiDocument.getDocLines().listIterator();
 boolean foundLn = false;
 while(li.hasNext() && !foundLn){
  DocLineBaseRec curr = li.next();
  if(Objects.equals(curr.getId(), arInvTblLineSel.getId())){
   li.remove();
   foundLn = true;
  }
 }
 LOGGER.log(INFO, "foundLn {0}", foundLn);
 if(foundLn){
  PrimeFaces pf = PrimeFaces.current();
...
public void onArInvPost(){
 ...
 LOGGER.log(INFO, "DocLineAr text {0}", docLineAr.getLineText());
 
 DocFiRec postingDoc = fiDocument;
 postingDoc.setDocInvoiceAr(invoice);
 UserRec crUsr = getLoggedInUser();
 Date currDate = new Date();
 postingDoc.setCreateOn(currDate);
 postingDoc.setCreatedBy(crUsr);
 postingDoc.setCompany(docCompany);
 postingDoc.setPartnerRef(custAccount.getArAccountCode());
 LOGGER.log(INFO, "Doc partner ref {0}", postingDoc.getPartnerRef());
 ...
 long lineNum = 0;
 LineTypeRuleRec lnTyGl = buffer.getLineTypeRuleByCode("GL");
 LineTypeRuleRec lnTyAr = buffer.getLineTypeRuleByCode("AR");
 ...
 // Fill missing fields in the document lines
 while(lnGLIt.hasNext()){
   lineNum++;
   
   DocLineBaseRec curr = lnGLIt.next();
   boolean debit = curr.getPostType().isDebit();
   LOGGER.log(INFO, "Process line {0} debit {1}", new Object[]{ lineNum, debit});
   curr.setId(null);
   ...
   switch(curr.getClass().getSimpleName()){
    case "DocLineGLRec":
     ...
     break;
    case "DocLineArRec" :
     curr.setLineType(lnTyAr);
     break;
   }
   lnGLIt.set(curr);
 }
 LOGGER.log(INFO, "posting doc lines after fill GL", postingDoc.getDocLines());
 if(netDocBal == 0){
  // document lines total zero cannot be posted
  MessageUtil.addClientWarnMessage("arDocCrSelFrm:msg", "docZero", "blacValidation");
  PrimeFaces.current().ajax().update("arDocCrSelFrm:msg");
  return;
 }
 // add AR line for customer account
 lineNum++;
 docLineAr.setLineNum(lineNum);
 
 docLineAr.setDocAmount(arDocAmnt);
 docLineAr.setHomeAmount(arDocAmnt);
 docLineAr.setClearedLine(false);
 docLineAr.setClearingLine(false);
 docLineAr.setCreateBy(crUsr);
 docLineAr.setCreateDate(currDate);
 docLineAr.setComp(postingDoc.getCompany());
 docLineAr.setDocHeaderBase(postingDoc);
 docLineAr.setArAccount(custAccount);
 
 PostTypeRec arPostTy;
 if(netDocBal > 0){
  arPostTy = buffer.getPostTypeForCode("arInv");
 }else{
  arPostTy = buffer.getPostTypeForCode("arCrn");
 }
 docLineAr.setPostType(arPostTy);
 if(docLineAr.getLineType() == null){
  docLineAr.setLineType(buffer.getLineTypeRuleByCode("AR"));
 }
 if(docLineAr.getPayType() == null){
  if(getPaymentTypes() == null || getPaymentTypes().isEmpty()){
   MessageUtil.addClientErrorMessage("arDocCrSelFrm:msg", "payTyNone", "errorText", this.docCompany.getReference());
   PrimeFaces.current().ajax().update("arDocCrSelFrm:msg");
   return;
  }
  PaymentTypeRec pt = getPaymentTypes().get(0);
  docLineAr.setPayType(pt);
 }
 List<DocLineGlRec> recLines = buildInvReconLines(docLineAr, fndbal, postingDoc, lnTyGl);
 LOGGER.log(INFO, "recLines {0}", recLines);
 docLineAr.setReconiliationLines(recLines);
 LOGGER.log(INFO,"docLineAr lineType before add to doc {0}",docLineAr.getLineType());
 LOGGER.log(INFO,"Line doc HDR usr {0}", docLineAr.getDocHeaderBase().getCreateOn());
 LOGGER.log(INFO,"docLineAr lineText {0}", docLineAr.getLineText());
 postingDoc.getDocLines().add(docLineAr);
 
 // logging parts of doc
 LOGGER.log(INFO, "Doc lines {0}", postingDoc.getDocLines());
 for(DocLineBaseRec curr:postingDoc.getDocLines() ){
   LOGGER.log(INFO,"Line num {0} type  {1}",curr.getLineType());
   
  }
 LOGGER.log(INFO, "vatSummary {0}", vatSummary);
 if(vatSummary == null || vatSummary.isEmpty()){
 ...
 for(DocLineBaseRec curr: postingDoc.getDocLines()){
   LOGGER.log(INFO, "Line num {0} class {1} ", 
     new Object[]{curr.getLineNum(),curr.getClass().getSimpleName()});
   LOGGER.log(INFO, "Post Key {0}", curr.getPostType().getPostTypeCode());
   LOGGER.log(INFO, "Line Type {0}", curr.getLineType().getLineCode());
  
   if(curr.getClass().getSimpleName().equals("DocLineArRec")){
    LOGGER.log(INFO, "ar rec acnt {0}", ((DocLineArRec)curr).getReconiliationLines());
   }
  }
 /*FiscalPeriodYearRec fisPerYr = buffer.getCompFiscalPeriodYearForDate(postingDoc.getCompany(), postingDoc.getPostingDate());
  postingDoc.setFisPeriod(fisPerYr.getPeriod());
  postingDoc.setFisYear(fisPerYr.getYear());
  */
  LOGGER.log(INFO, "Web about to call postArInv arLine text  {0} ", docLineAr.getLineText());
  LOGGER.log(INFO, "fisc per {0} year {1}", new Object[]{postingDoc.getFisPeriod(), postingDoc.getFisYear()});
  
  postingDoc = docMgr.postArInv(postingDoc, vatSummary,fndbal, getView());
  ...
  if(postingDoc.getId() == null){
   ...
   switch(getViewSimple()){
    case "arInvPost":
     fiDocument = postingDoc;
     MessageUtil.addClientInfoMessage("arDocCrSelFrm:grl", "arInvPosted", "blacResponse", 
     String.valueOf(fiDocument.getDocNumber()));
     fiDocument = null;
     docCompany = getCompList().get(0);
     arDocAmnt = 0.0;
     custAccount = null;
     ...
     break;
    case "arCrnPost":
     LOGGER.log(INFO, "Credit note posted");
     break;
   }
  }
 /*
 ...
 while(linesLi.hasNext()){
  ...
  if(StringUtils.equals(curr.getClass().getSimpleName(), "DocLineGlRec")){
   // adding GL line
   DocLineGlRec currGlLn = (DocLineGlRec)curr;
   if(!lnVat){
   lnVat = currGlLn.getGlAccount().getVatCode() != null;
   }
   LOGGER.log(INFO, "GL account {0} vatable lines {1}", new Object[]{
   ((DocLineGlRec)curr).getGlAccount().getCoaAccount().getRef(), lnVat});
   
   if(currGlLn.getRestrictedFund() != null){
    LOGGER.log(INFO, "Restricted Fund {0}", currGlLn.getRestrictedFund().getFndCode());
    FundRec lnFund = currGlLn.getRestrictedFund();
    if(fundBal == null){
     fundBal = new ArrayList<>();
     
    }
    if(fundBal.isEmpty()){
      FundBalance bal = new FundBalance();
      bal.setFund(lnFund);
      bal.setAmount(currGlLn.getDocAmount());
      fundBal.add(bal);
    }else{
     // need to add to balance for fund
     boolean foundFund = false;
     ListIterator<FundBalance> fndLi = fundBal.listIterator();
     while(fndLi.hasNext() && !foundFund){
      FundBalance currFnd = fndLi.next();
      if(Objects.equals(currFnd.getFund().getId(), lnFund.getId())){
       // found bal line to update
       double fndAmnt = currFnd.getAmount();
       if(debit){
        fndAmnt += currGlLn.getDocAmount();
       }else{
        fndAmnt -= currGlLn.getDocAmount();
       }
       currFnd.setAmount(fndAmnt);
       fndLi.set(currFnd);
      }
     }
     if(!foundFund){
   ...
   currGlLn.setDocHeaderBase(fiDocument);
   linesLi.set(currGlLn);
   
  }
  // add to fund balance if there is a fund of the GL line
 ...
 fiDocument = docMgr.postArInv(fiDocument, docVatSummary, fundBal, getView());//docMgr.updateDocument(fiDocument, this.getLoggedInUser(), getView());
  ...
  if(input == null || input.isEmpty()){
   ...
   for(PostTypeRec pt:getPostTypes()){
    if((pt.getLedger().getCode().equals("GL") || pt.getLedger().getCode().equals("AR")) &&
      StringUtils.startsWithIgnoreCase(pt.getDescription(), input)  && !pt.isSysUse()){
     retList.add(pt);
    }
   }
   if(retList.isEmpty()){
...
public void onCompanySel(SelectEvent evt){
 ...
 FiscalPeriodYearRec fiscPer = buffer.getCompFiscalPeriodYearForDate(docCompany, fiDocument.getPostingDate());
 LOGGER.log(INFO, "fiscPer year {0} period {1}", new Object[]{fiscPer.getYear(),fiscPer.getPeriod()});
 fiDocument.setFisPeriod(fiscPer.getPeriod());
 fiDocument.setFisYear(fiscPer.getYear());
 fiDocument.setFisYearPeriod(fiscPer.getYearPeriod());
 PrimeFaces.current().ajax().update("arDocCrDetFrm:postPer");
 ...
 public void onPostDateSelect(SelectEvent event){
  ...
  fiDocument.setFisPeriod(perYr.getPeriod());
  fiDocument.setFisYear(perYr.getYear());
  fiDocument.setFisYearPeriod(perYr.getYearPeriod());
  PrimeFaces.current().ajax().update("arDocCrDetFrm:postPer");
 ...
 public List<PostTypeRec> onPostKeyComplete(String input){
  ...
  if(input == null || input.isEmpty()){
   ...
   for(PostTypeRec pt:getPostTypes()){
    LOGGER.log(INFO, "Post type {0} sys use {1}", new Object[]{pt.getPostTypeCode(),pt.isSysUse()});
    if(StringUtils.startsWithIgnoreCase(pt.getDescription(),input) && !pt.isSysUse()){
     retList.add(pt);
    }
   }
