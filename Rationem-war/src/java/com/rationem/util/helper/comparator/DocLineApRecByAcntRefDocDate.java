 ...
 * @author user
 */
public class DocLineApRecByAcntRefDocDate implements Comparator<DocLineApRec> {
 
 @Override
 public int compare(DocLineApRec ln1,DocLineApRec ln2){
  int rslt = ln1.getAccountRef().compareTo(ln2.getAccountRef());
  if(rslt == 0){
   return ln1.getDocFi().getDocumentDate().compareTo(ln2.getDocFi().getDocumentDate());
  }else{
   return rslt;
  }
 }
